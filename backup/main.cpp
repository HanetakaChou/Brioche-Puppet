...
extern bool _internal_platform_get_open_file_name(void *platform_context, uint32_t filter_count, char const *const *filter_names, char const *const *filter_specs, int &inout_file_type_index, mcrt_string *out_file_name, uint64_t *out_file_timestamp, mcrt_vector<uint8_t> *out_file_data);
...
#endif
{
    ...
    brx_wsi_init_main_window("Brioche Puppet", reinterpret_cast<void *>(s_main_wnd_proc), s_wsi_state.m_window_width, s_wsi_state.m_window_height);
    ...
    // Run
    while (s_wsi_state.m_running)
    {
        brx_wsi_wait_window();

        // Render
        if (s_wsi_state.m_running && s_wsi_state.m_window_width > 0 && s_wsi_state.m_window_height > 0)
        {
            float interval_time;
            {
...
static LRESULT CALLBACK s_main_wnd_proc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
    {
        ...
        s_wsi_state.m_ui_controller.m_first_person_camera.HandleMouseMoveMessage(normalized_x, normalized_y, leftButton, middleButton, rightButton);
    }
        return 0;
    case WM_ERASEBKGND:
        return 1;
    case WM_KEYDOWN:
    {
        ...
        s_wsi_state.m_ui_controller.m_first_person_camera.HandleKeyDownMessage(mappedKey);
    }
        return 0;
    case WM_KEYUP:
    {
        ...
        s_wsi_state.m_ui_controller.m_first_person_camera.HandleKeyUpMessage(mappedKey);
    }
        return 0;
    case WM_SIZE:
    {
